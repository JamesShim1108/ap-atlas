...
function pageTitle(title,description){document.title=`${title} — AP Atlas`;document.querySelector('meta[name="description"]').content=description;}
...
function home(){
 pageTitle('Study smarter for AP','Free, focused AP study. Learn concepts, practice with original questions, and find what to review next.');
 return `<div class="container"><section class="home-top"><div><p class="eyebrow">A clearer way to study</p><h1>Study smarter<br>for <em>AP.</em></h1><p class="home-intro">Learn concepts. Review what matters. Practice until it sticks.</p>${link('/courses',`Start studying ${arrow}`,'btn')}<p class="free-note">Free access. No signup. Just start.</p></div><article class="course-feature"><div class="feature-topline"><span class="eyebrow" style="margin:0">Your first course</span>${badge('ready','START HERE')}</div><h2>AP World History:<br>Modern</h2><p class="period">c. 1200–present</p><hr><p class="feature-info">Start with Unit 1 · The Global Tapestry<br>Lesson, vocabulary, and practice in one place.</p>${link(`/course/${firstCourse.id}`,`Explore AP World ${arrow}`,'btn')}<p class="feature-foot">Topic 1.1 available now. More topics to come.</p></article></section><section class="how-section"><div class="section-heading"><h2>A little learning. A clearer next step.</h2><small class="muted">Make your study time count.</small></div><div class="step-grid"><div class="step"><div class="step-index">01</div><div><h3>Understand the idea</h3><p>Short explanations, useful terms, and connections that make the facts stick.</p></div></div><div class="step"><div class="step-index">02</div><div><h3>Put it into practice</h3><p>One question at a time, with an explanation after every answer.</p></div></div><div class="step"><div class="step-index">03</div><div><h3>Know what to revisit</h3><p>See which concepts went well and which deserve another look.</p></div></div></div></section></div>`;
}
function courseList(){pageTitle('Courses','Choose an AP course. Start with AP World History: Modern and an original East Asia lesson.');return `<div class="container">${crumb([['Home','/'],['Courses']])}<div class="page-intro"><p class="eyebrow">Choose your starting point</p><h1>Your course. Your pace.</h1><p>Start with a topic, understand the connections, and test what you know.</p></div><div class="courses-list">${courses.map(c=>`<a class="course-list-card" href="#/course/${esc(c.id)}"><div class="course-monogram" aria-hidden="true">W</div><div>${badge(c.status)}<h2>${esc(c.title)}</h2><p>${esc(c.description)}</p><p style="margin-top:.8rem;font-size:.85rem">Unit 1 · Topic 1.1 ready to study</p></div><span class="big-arrow" aria-hidden="true">↗</span></a>`).join('')}</div></div>`;}
...
function quizPage(q){
 ...
 if(!a)return `<div class="quiz-page">${crumb(topicCrumbs(t,'Quiz'))}<div class="empty-state"><p class="eyebrow">Topic ${t.code}</p><h1>Ready when you are.</h1><p>Eight questions, one at a time. Read the explanation after each answer and see what to review at the end.</p>${button(`Start quiz ${arrow}`,'start',q.id)}</div></div>`;
...
function comingSoon(title,back){pageTitle(title+' — Coming soon','This AP Atlas content is being prepared. Topic 1.1 is available now.');return `<div class="empty-state"><p class="eyebrow">Coming soon</p><h1>${esc(title)}</h1><p>This material isn’t available yet. You can study East Asia and take the Topic 1.1 quiz now.</p>${link(`/topic/${firstTopic.id}`,`Study Topic ${firstTopic.code} ${arrow}`,'btn')}<p style="margin-top:1.5rem">${link(back,'Back to the overview')}</p></div>`;}
function missing(){pageTitle('Page not found','Choose a course to continue studying on AP Atlas.');return `<div class="empty-state"><h1>Let’s get you back on track.</h1><p>We couldn’t find that page. Pick a course to continue.</p>${link('/courses',`Browse courses ${arrow}`,'btn')}</div>`;}
function render(focus=false){
 ...
 $('#main').innerHTML=html;
 $('#nav-courses').toggleAttribute('aria-current',parts[0]==='courses');
 if(parts[0]==='courses')$('#nav-courses').setAttribute('aria-current','page');
